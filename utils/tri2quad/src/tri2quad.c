/* Convert triangular mesh to quadrilateral mesh by splitting */
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    fprintf(stderr,"\n");
    fprintf(stderr,"usage: tri2quad infilename outfilename\n\n");
    exit(-1);
  }
  ...
  // Call the Tri to Quad conversion routine. Each edge is split in two
  ...
  // midpoints of edges and a vertex of the triangle;

  Mesh_ptr quadmesh;
  ok = MESH_Tri2Quad(mesh, &quadmesh);
  if (!ok) {
    fprintf(stderr, "Tri->Quad conversion failed. Exiting...\n");
    exit(-1);
  }
  ...
  if (outmeshformat == MSTK) {
    ...
    MESH_WriteToFile(quadmesh, outfname, MESH_RepType(mesh), comm);
    fprintf(stderr,"Done\n");
  }
  else {
    ...
    case GMV:
      ...
      ok = MESH_ExportToFile(quadmesh, outfname, "gmv", -1, NULL, NULL, comm);
      break;
    case EXODUSII:case NEMESISI:
      if (rank == 0)
        fprintf(stderr,"Exporting mesh to ExodusII/NemesisI format...");
      ok = MESH_ExportToFile(quadmesh, outfname, "exodusii", -1, NULL, NULL,
			     comm);
      break;
    case X3D:
      if (rank == 0)
        fprintf(stderr,"Exporting mesh to FLAG/X3D format...");
      ok = MESH_ExportToFile(quadmesh, outfname, "x3d", -1, NULL, NULL, comm);
      break;
    default:
      if (rank == 0)
