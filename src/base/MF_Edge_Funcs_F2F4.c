...
#ifdef DEBUG
    if (n > 32)
      MSTK_Report("MF_Set_Edges_F2F4","Currently, only 32 edges supported per face",ERROR);
#endif
...
/*   } */
  ...
  void MF_Replace_Edges_i_F2F4(MFace_ptr f, int nold, int i, int nnu, 
			     MEdge_ptr *nuedges) {
    MFace_Adj_F2F4 *adj;
    ...
    for (j = 0; j < nnu; j++) {
	  ...
	  MSTK_Report("MF_Replace_Edges_i_F2F4",
		      "Face and edge belong to different meshes",FATAL);
#endif
    }
  ...
  List_ptr MF_Edges_F2F4(MFace_ptr f, int dir, MVertex_ptr v0) {
  ...
  int MF_EdgeDir_F2F4(MFace_ptr f, MEdge_ptr e) {
    ...
    MSTK_Report("MF_Edges_F2F4","Cannot find edge in face!!",FATAL);
