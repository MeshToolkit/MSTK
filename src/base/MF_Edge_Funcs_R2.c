...
extern "C" {
  ...
  int MF_Num_Edges_R2(MFace_ptr f) {
    return List_Num_Entries(((MFace_Adj_R2 *)f->adj)->fvertices);
  }
  ...
  List_ptr MF_Edges_R2(MFace_ptr f, int dir, MVertex_ptr v0) {
	...
	if (v == v0)
      ...
      if (!fnd) {
	MSTK_Report("MF_Edges_R2","Cannot find vertex in face",MSTK_ERROR);
	return NULL;
      }
    }
  ...
  void MF_EdgeIDs_R2(MFace_ptr f, int dir, int startvertid, int *nfe,
                     int *fedgeids) {
    *nfe = 0;
    MSTK_Report("MF_EdgeIDs_R2","NOT IMPLEMENTED - No persistent edges in representation",MSTK_WARN);
  }
