...
extern "C" {
  ...
  */
  ...
  int MESH_Vol_Partition_Some(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes);
  ...
  int MESH_Surf_Partition_Some_FN(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes);
  int MESH_Vol_Partition_Some_FN(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes);
  int MESH_Surf_Partition_Some_R1R2R4(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes);
  int MESH_Vol_Partition_Some_R1R2(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes);
  int MESH_Vol_Partition_Some_R4(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes);
  ...
  static int (*MESH_Vol_Partition_Some_jmp[MSTK_MAXREP])(Mesh_ptr mesh, int num,
                            int *part, int begin, int end, 
						    Mesh_ptr *submeshes) =
  {MESH_Vol_Partition_Some_FN, MESH_Vol_Partition_Some_FN, MESH_Vol_Partition_Some_R1R2,
   MESH_Vol_Partition_Some_R1R2, MESH_Vol_Partition_Some_R4};
  

  int MESH_Partition_Some(Mesh_ptr mesh, int num_total, int *part,
                          int begin, int end, Mesh_ptr *submeshes) {
    ...
    if (!nr && !nf) {
      MSTK_Report("MESH_Partition_Some",
		  "This is not a valid mstk file for partition-some",MSTK_ERROR);
      exit(-1);
    }

    if (nr) {
      ok = MESH_Vol_Partition_Some(mesh,num_total,part,begin,end,submeshes);
    }
    else if (nf) {
      MSTK_Report("MESH_Partition_Some",
		  "This is not a valid mstk file for partition-some",MSTK_ERROR);
  ...
  int MESH_Vol_Partition_Some(Mesh_ptr mesh, int num_total, int *part,
                          int begin, int end, Mesh_ptr *submeshes) {
    RepType rtype = MESH_RepType(mesh);
    int i;
    ...
    for (i = 0; i < end - begin; i++) {
      submeshes[i] = MESH_New(rtype);
      MESH_Set_Prtn(submeshes[i],i+begin,num_total);
    }

    return (*MESH_Vol_Partition_Some_jmp[rtype])(mesh, num_total, part, begin, end, submeshes);
  }
...
int MESH_Vol_Partition_Some_FN(Mesh_ptr mesh, int num_total, int *part, int begin, int end, Mesh_ptr *submeshes) {
    int i, j, ic, idx, idx2, mpart_no;
    int nv, nr, nrv, nre, nrf, nfe;
    int grid=1, gfid=1, geid=1, gvid=1;
    double xyz[3];
    ...
    /* for each partition */
    for (part_no = begin; part_no < end; part_no++) {
      part_index = part_no - begin;

      l2gatt = MAttrib_New(submeshes[part_index],"Local2Global",POINTER,MALLTYPE);
      ...
      while ((gmr = MESH_Next_Region(mesh,&idx))) {
        ...
        for (i = 0; i < nrf; i++) {
          ...
          if (!lmflist) {
            ...
            idx2 = 0;
            while ((lmf = List_Next_Entry(lmflist,&idx2))) {
              if (MF_Mesh(lmf) == submeshes[part_index]) {
                found_lmf = 1;
                break;
              }
            }
          ...
          if (!found_lmf) {
            lmf = MF_New(submeshes[part_index]);
            MF_Set_GEntID(lmf,MF_GEntID(gmf));
            MF_Set_GEntDim(lmf,MF_GEntDim(gmf));
            mpart_no = MF_MasterParID(gmf);
            if (mpart_no == 0) { /* not set */
            ...
            for (j = 0; j < nfe; j++) {
              ...
              if (!lmelist) {
                ...
                idx2 = 0;
                while ((lme = List_Next_Entry(lmelist,&idx2))) {
                  if (ME_Mesh(lme) == submeshes[part_index]) {
                    found_lme = 1;
                    break;
                  }
                }
              ...
              if (!found_lme) {
                lme = ME_New(submeshes[part_index]);
                ME_Set_GEntID(lme,ME_GEntID(gme));
                ME_Set_GEntDim(lme,ME_GEntDim(gme));
                mpart_no = ME_MasterParID(gme);
                if (mpart_no == 0) { /* not set */
                ...
                for (k = 0; k < 2; k++) {
                  ...
                  if (!lmvlist) {
                    ...
                    idx2 = 0;
                    while ((lmv = List_Next_Entry(lmvlist,&idx2))) {
                      if (MV_Mesh(lmv) == submeshes[part_index]) {
                        found_lmv = 1;
                        break;
                      }
                    }
                  ...
                  if (!found_lmv) {
                    lmv = MV_New(submeshes[part_index]);
                    MV_Coords(gmv,xyz);
                    MV_Set_Coords(lmv,xyz);
                    MV_Set_GEntID(lmv,MV_GEntID(gmv));
                    MV_Set_GEntDim(lmv,MV_GEntDim(gmv));
        ...
        lmr = MR_New(submeshes[part_index]);
        MR_Set_GEntID(lmr,MR_GEntID(gmr));
        MR_Set_GEntDim(lmr,MR_GEntDim(gmr));
        //	MR_Set_PType(lmr,PINTERIOR);
        MR_Set_MasterParID(lmr,part_no);
...
int MESH_Surf_Partition_Some_R1R2R4(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes) {
    MSTK_Report("MESH_Surf_Partition_Some_R1R2R4","Not implemented",MSTK_FATAL);
    return 0;
  }

    
  int MESH_Vol_Partition_Some_R1R2(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes) {
    MSTK_Report("MESH_Vol_Partition_Some_R1R2","Not implemented",MSTK_FATAL);
    return 0;
  }


  int MESH_Vol_Partition_Some_R4(Mesh_ptr mesh, int num, int *part, int begin, int end, Mesh_ptr *submeshes) {
    MSTK_Report("MESH_Vol_Partition_Some_R4","Not implemented",MSTK_FATAL);
    return 0;
  }
